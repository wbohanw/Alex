...
// Adapted from React Bits Particle Text: https://reactbits.dev/text-animations/particle-text
...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async (): Promise<void> => {
      ...
      offCtx.font = font;
      let lineMetrics = lines.map(line => offCtx.measureText(line));
      const measuredWidth = Math.max(1, ...lineMetrics.map(metrics => metrics.width));
      if (measuredWidth > maxTextWidth) {
        ...
        if (currentBuild !== buildId) return;
        offCtx.font = font;
        lineMetrics = lines.map(line => offCtx.measureText(line));
      }
      ...
      const lineHeight = Math.max(1, ascent + descent + resolvedSize * 0.14);
      ...
      lines.forEach((line, index) => {
        offCtx.fillText(line, offscreen.width / 2, padding + lineHeight * (index + 0.5));
      });
